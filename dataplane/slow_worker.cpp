...
SlowWorker::SlowWorker(cWorker* worker,
        ...
        m_workers_serviced{workers_to_service},
        from_gcs_{from_gcs},
        ...
        m_fragmentation(
                ...
                m_slow_worker->dataPlane->getConfigValues().fragmentation_size),
        m_dregress(this,
        ...
        m_config{sw_icmp_out_rate_limit, use_kni},
        m_kni_worker{std::move(kni)}
{
	m_workers_serviced.emplace_back(m_slow_worker);
}

SlowWorker::SlowWorker(SlowWorker&& other) :
        m_run_barrier{other.m_run_barrier},
        m_ports_serviced{std::move(other.m_ports_serviced)},
        m_workers_serviced{std::move(other.m_workers_serviced)},
        m_slow_worker{other.m_slow_worker},
        m_mempool{other.m_mempool},
        m_fragmentation{std::move(other.m_fragmentation)},
        m_dregress{std::move(other.m_dregress)},
        m_kni_worker{std::move(other.m_kni_worker)}
{
	m_fragmentation.Callback() = [this](rte_mbuf* pkt, const common::globalBase::tFlow& flow) {
		sendPacketToSlowWorker(pkt, flow);
	};
}
